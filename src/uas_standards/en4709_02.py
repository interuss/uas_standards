...
class OperatorRegistrationNumber(str):
    ...
    def make_invalid_by_changing_final_control_string(
        ...
        while True:
            new_random_string = "".join(
                r.choice(string.ascii_lowercase) for _ in range(OperatorRegistrationNumber.final_random_string_length)
            )
            result = OperatorRegistrationNumber(self.checksum_control + "-" + new_random_string)
            if not result.valid:
                return result
    ...
    def generate_valid(prefix: str, r: Optional[random.Random] = None) -> OperatorRegistrationNumber:
        ...
        base_id = "".join(
            r.choice(string.ascii_lowercase + string.digits) for _ in range(OperatorRegistrationNumber.base_id_length)
        )
